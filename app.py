...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0a2540 0%, #0d3b6e 100%);
    border-right: none;
}
...
Once you have ALL 6 details, end your message with this EXACT block (replace values):
...
with st.sidebar:
    ...
    st.markdown('<div class="info-card"><h4>✅ Insurance</h4><p>', unsafe_allow_html=True)
    ...
    # Language selector
    st.markdown('<p class="lang-selector-label">🌐 Language / Idioma</p>', unsafe_allow_html=True)
...
if not st.session_state.messages:
    welcome = {
        ...
        "French": "Bonjour! Bienvenue chez Bright Smile Dental! 😊 Je suis votre réceptionniste virtuelle, disponible 24h/24. Comment puis-je vous aider?",
    ...
    if message["role"] == "user":
        ...
        if "[APPOINTMENT_BOOKED]" in content:
            ...
            try:
                ...
                if details and not st.session_state.booking_done:
                    save_appointment(
                        details.get("Name", "Unknown"),
                        details.get("Date", "TBD"),
                        details.get("Time", "TBD"),
                        details.get("Service", "General"),
                        details.get("Phone", "N/A"),
                        details.get("Insurance", "N/A")
                    )
        ...
        with st.chat_message("assistant", avatar="🦷"):
            ...
            if "[APPOINTMENT_BOOKED]" in content:
                try:
                    block = content.split("[APPOINTMENT_BOOKED]")[1].split("[/APPOINTMENT_BOOKED]")[0].strip()
                    lines = block.strip().split("\n")
                    details = {}
                    for line in lines:
                        if ":" in line:
